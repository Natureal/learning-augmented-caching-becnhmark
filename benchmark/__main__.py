...
if __name__ == "__main__":
    ...
    mode_group.add_argument('--lrb', action='store_true')
    mode_group.add_argument('--lrb_fr', action='store_true')
    mode_group.add_argument('--lrbcomplete', action='store_true')
    mode_group.add_argument('--lrbcomplete_fr', action='store_true')
    mode_group.add_argument('--guard_lrb', action='store_true')
    ...
    parser.add_argument("--relaxation_factor", type=float, default=10.0)
    parser.add_argument("--admission_size", type=int, default=None)
    parser.add_argument("--lrb_only", action="store_true")
    parser.add_argument("--disable_admission", action="store_true")
    parser.add_argument("--disable_edc", action="store_true")
    parser.add_argument("--debug", action="store_true")
    parser.add_argument("--include_guard_lrb", action="store_true")
    parser.add_argument("--include_simple_guard_lrb", action="store_true")
    
    parser.add_argument("--quiet", action='store_true')

    parser.add_argument("--relax_threshold", type=float, default=0.2)
    parser.add_argument("--enable_random_relax", action="store_true")
    parser.add_argument("--phase_reset_percentage", type=float, default=0.7)
    parser.add_argument("--guard_weight", type=float, default=0.7)

    parser.add_argument("--simple_relax_times", type=int, default=0)
    parser.add_argument("--simple_relax_prob", type=float, default=0.0)
    ...
    if args.lrbcomplete or args.lrbcomplete_fr:
        ...
        with DataTrace(file_path) as trace:
            with tqdm.tqdm(desc="SimpleGuardLRB", disable=disable_progress) as pbar:
                while not trace.done():
                    pc, address = trace.next()
                    cache.access(pc, address)
                    pbar.update(1)
        ...
        hit, miss, total, rate = cache.stat()
        table = PrettyTable() 
        table.field_names = ["algo", "hit", "miss", "total", "hit_rate"]
        ...
        if args.real:
            ...
            if 'pleco' in this_preds:
                ...
                online_types.extend([
                    ...
                    PredictAlgorithmFactory.generate_predictive_algorithm(partial(PredictiveRPBNewOnlineMinAlgorithm, pred_budget=4, max_support_factor=100), 'PLECO'),
                    PredictAlgorithmFactory.generate_predictive_algorithm(partial(PredictiveRPBNewOnlineMinAlgorithm, pred_budget=8, max_support_factor=100), 'PLECO'),
                    PredictAlgorithmFactory.generate_predictive_algorithm(partial(PredictiveRPBNewOnlineMinAlgorithm, pred_budget=100, max_support_factor=100), 'PLECO'),
            ...
            if 'popu' in this_preds:
                ...
                online_types.extend([
                    ...
                    PredictAlgorithmFactory.generate_predictive_algorithm(partial(PredictiveRPBNewOnlineMinAlgorithm, pred_budget=100, max_support_factor=100), 'POPU'),
            ...
            if 'lrb' in this_preds:
                ...
                if PredictAlgorithmFactory.include_lrb_variants:
                    online_types.extend([
                        ...
                        PredictAlgorithmFactory.generate_predictive_algorithm(partial(PredictiveRPBNewOnlineMinAlgorithm, pred_budget=0, max_support_factor=100), 'LRB', shared_model=lrb_gen(), memory_window=args.memory_window),
                        PredictAlgorithmFactory.generate_predictive_algorithm(partial(PredictiveRPBNewOnlineMinAlgorithm, pred_budget=1, max_support_factor=100), 'LRB', shared_model=lrb_gen(), memory_window=args.memory_window),
                        PredictAlgorithmFactory.generate_predictive_algorithm(partial(PredictiveRPBNewOnlineMinAlgorithm, pred_budget=2, max_support_factor=100), 'LRB', shared_model=lrb_gen(), memory_window=args.memory_window),
                        PredictAlgorithmFactory.generate_predictive_algorithm(partial(PredictiveRPBNewOnlineMinAlgorithm, pred_budget=4, max_support_factor=100), 'LRB', shared_model=lrb_gen(), memory_window=args.memory_window),
                        PredictAlgorithmFactory.generate_predictive_algorithm(partial(PredictiveRPBNewOnlineMinAlgorithm, pred_budget=8, max_support_factor=100), 'LRB', shared_model=lrb_gen(), memory_window=args.memory_window),
                        PredictAlgorithmFactory.generate_predictive_algorithm(partial(PredictiveRPBNewOnlineMinAlgorithm, pred_budget=100, max_support_factor=100), 'LRB', shared_model=lrb_gen(), memory_window=args.memory_window),
            ...
            if 'oracle_dis' in this_preds:
                oracle_types.extend([
                    ...
                    (partial(PredictiveRPBNewOnlineMinAlgorithm, pred_budget=4, max_support_factor=100), 'OracleDis'),
                    (partial(PredictiveRPBNewOnlineMinAlgorithm, pred_budget=8, max_support_factor=100), 'OracleDis'),
                    (partial(PredictiveRPBNewOnlineMinAlgorithm, pred_budget=100, max_support_factor=100), 'OracleDis'),
                ])
                combiner_types.extend([
            ...
            if 'oracle_bin' in this_preds:
                oracle_types.extend([
                    ...
                    (partial(PredictiveRPBNewOnlineMinAlgorithm, pred_budget=0, max_support_factor=100), 'OracleBin'),
                    (partial(PredictiveRPBNewOnlineMinAlgorithm, pred_budget=1, max_support_factor=100), 'OracleBin'),
                    (partial(PredictiveRPBNewOnlineMinAlgorithm, pred_budget=2, max_support_factor=100), 'OracleBin'),
                    # (partial(PredictiveRPBNewOnlineMinAlgorithm, pred_budget=4, max_support_factor=100), 'OracleBin'),
                    # (partial(PredictiveRPBNewOnlineMinAlgorithm, pred_budget=8, max_support_factor=100), 'OracleBin'),
                    # (partial(PredictiveRPBNewOnlineMinAlgorithm, pred_budget=100, max_support_factor=100), 'OracleBin'),
...
###############################################################
    ...
    lru_miss = np.inf
    if 'OPT' in cache_dict:
        hit, lru_miss, total, rate = cache_dict['LRU'][0].stat()

    if verbose:
        table.field_names = ["Name", "Hit", "Miss", "Total", "Hit Rate", "Cost Ratio", "LRU-normalized Cost Ratio"]
        for i, (pretty_name, _, _) in enumerate(funcs):
            hit, miss, total, rate = caches[i].stat()
            table.add_row([pretty_name, hit, miss, total, rate, f"{miss / opt_miss:.3f}", f"{(miss - opt_miss)/(lru_miss - opt_miss):.3f}"])
        if sorted:
